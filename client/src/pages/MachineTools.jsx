...
export default function MachineTools() {
    ...
    return <div className="container manager-page manager-equipment-page">
        <PageHeader eyebrow={getSessionUser()?.role === 'admin' ? 'Admin operations' : 'Manager workspace'} title="Equipment" description="Manage equipment and open related maintenance requests." actions={<div className="manager-equipment-actions"><input className="manager-equipment-search" type="search" placeholder="Search equipment" value={query} onChange={(e) => setQuery(e.target.value)} aria-label="Search equipment" />{canManage && <Button onClick={openCreate}>Add equipment</Button>}</div>} />
